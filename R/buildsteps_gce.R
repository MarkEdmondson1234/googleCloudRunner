...
cr_buildstep_compute_container <- function(
  vm_name,
  ...
  cr_buildstep_gcloud(
    args = c(
      ...
      sprintf("--container-mount-disk=name=%s,mount-path=%s",
...
cr_buildstep_compute_rstudio <- function(
  ...
  cr_buildstep_compute_container(
     ...
     container_env = sprintf("--container-env=[ROOT=TRUE,USER=%s,PASSWORD=%s]",
