...
#' @param footer should a comment footer with the time be added?
#'
#' @export
#' @family Cloud Build functions
#' @examples
...
cr_build_write.gar_Build <- function(x, file = "cloudbuild.yaml",
  ...
  cr_build_write.cr_yaml(o, file, footer = footer)
  file
}
...
cr_build_write.cr_yaml <- function(x, file = "cloudbuild.yaml",
  ...
  if (footer) {
    write(
      paste("#Generated by googleCloudRunner::cr_build_write at", Sys.time()),
