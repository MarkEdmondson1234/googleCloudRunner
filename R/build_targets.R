...
#'
#' @param create_dag If TRUE will create a parallel running buildstep DAG for the Cloud Build to improve efficiency
#' @param ... Other arguments passed to \link{cr_build_yaml}
#' @inheritDotParams cr_build_yaml
...
cr_build_targets <- function(
  ...
  if(create_dag){
    bs <- cr_build_targets_dag("${_TARGET_BUCKET}",
...
cr_build_targets_dag <- function(
  ...
  c(
    ...
    bst,
    cr_buildstep_targets_teardown(bucket_folder,
                                  last_id = last_id)
  )
}
