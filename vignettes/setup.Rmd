...
"us-east1"
```
* New environment argument `CR_BUILD_EMAIL` that is a Google service email
...
The suggested setup is only required once per GCP project instance.  Following the below instructions will mean:
...
7. Download the auth json key and put it in a folder separate from all R projects or git commits.  **This file should be guarded as it grants access to your projects**
8. Set your `.Renviron` file in your project or R home to point at your file and use the email for scheduled builds e.g.:
...
The above could be modified to make the local auth file also give access to compute engine VMs etc. if say you wanted to reuse it for `googleComputeEngineR` but this should be the minimal access you need for `googleCloudRunner`.  

You could also change the email to yet another service account in `CR_BUILD_EMAIL` to one with less permissions (such as Cloud Run Invoker, not Cloud Run Admin)

#### The Cloud Build service account

This is the service account for Cloud Build when it is running on your project.  It looks like `{project-number}@cloudbuild.gserviceaccount.com`
