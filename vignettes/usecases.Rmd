...
#### Call the private plumber API with a JWT
...
library(httr)
res <- cr_jwt_with(GET("https://parallel-cloudrun-ewjogewawq-ew.a.run.app/"),
                   token)
content(res)
```
...
For parallel processing, the [`future.apply`]( https://CRAN.R-project.org/package=future.apply) package has a drop in replacement for `lapply()` - this is used to call the API with many threads simultaneously: 
