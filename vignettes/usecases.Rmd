...
r_code <- cr_build_yaml(
  cr_buildstep("gcr.io/my-project/my-r-script", args = "my-script.R")
)
build <- cr_build(r_code)
built <- cr_build_wait(build)
```

That is, run your container with a CMD pointing at your R script.  You could of course add other steps if you wanted, such as sending an email when done via `cr_buildstep_mailgun()`.
