...
## Build and deploy Rmd files to Cloud Run on each git commit
...
This can be achieved following the below:

1. Mirror your GitHub/Bitbucket repo to Cloud Repositories
2. Put your Rmarkdown files in a directory within your Git repo 
3. Create a build that will:
...
```r
r <- "rmarkdown::render('cloudrun_rmarkdown.Rmd', output_file = 'cloudrun_rmarkdown.html')"

build_rmd <- cr_build_make(
    cr_build_yaml(
      steps = c(
          cr_buildstep_r(
            id="render rmd",
          ...
          cr_buildstep_nginx_setup("inst/cloudrun_rmarkdown/"),
          cr_buildstep_docker("html-on-cloudrun",
                              tag = "$BUILD_ID",
                              dir = "inst/cloudrun_rmarkdown/"),
...
cr_build_write(build_rmd, "inst/cloudrun_rmarkdown/cloudbuild_rmd_run.yml")
```

5. Commit the configured cloudbuild yaml file to your repo
6. Create a Build Trigger that builds your cloudbuild yaml file
