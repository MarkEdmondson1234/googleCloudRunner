...
Connecting Cloud Build and git allows you to create workflows that automate tasks upon each git commit, via Build Triggers. 

A rundown on some common workflows and connections are detailed here.

## Cloud Build Triggers and GitHub App

Cloud Builds can be triggered via Build Triggers, and those Build Triggers can be initiated by GitHub, Bitbucket or Cloud Source Repository events such as commits or pull requests.

For buildtriggers, you create the git settings via `cr_buildtrigger_repo()` which specifies if it is a GitHub repo, or a Cloud Source repository (that may be a mirror of another git service)
...
`cr_buildstep_gitsetup()` calls `cr_buildstep_secret()` when you supply it the Secret Manager name:
