...
vignette: >
  %\VignetteIndexEntry{Serverless batched R scripts via Cloud Build}
...
Instead of using seperate `cloudbuild.yml` files, you can also choose to make your own cloudbuild.yaml files in R via `cr_build_yaml()` and `cr_buildstep()`
...
1. Link your repo (GitHub, Bitbucket or Cloud Repositores) to Google 
...
You can setup either Docker builds by just providing the location of the `Dockerfile`, or have more control by providing a `cloudbuild.yml` - by defualt these are looked for in the root directory.
...
### Build Triggers via code (Experimental)

> This API is experimental, so may not be reliable for production
...
cr_buildtrigger("trig3", trigger = github, build = "cloudbuild.yaml")
```
...
Mirroring the GitHub repo is necessary if using it for sources of builds.  
