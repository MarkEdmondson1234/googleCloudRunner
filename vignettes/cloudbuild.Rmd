...
Below is an example that uses `library(googlePubSubR)` to create and send a message to PubSub, but any other service that publishes to PubSub could be used.
...
You should now see the Cloud build start in your console.  You can check the logs for the build after it has run to see if it worked:
...
See `cr_schedule_pubsub()` which can use PubSub to trigger scheduled builds.  There is also a guide using [Cloud Scheduler to trigger Build Triggers via Pubsub](articles/cloudscheduler.html) 
