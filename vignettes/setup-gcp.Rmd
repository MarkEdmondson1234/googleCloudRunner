...
* Scopes - you can skip this for now as it can be added later via `cr_setup()` or the manual setup instructions below.  As the app will not be public, the app should not require verification
* Test users = "your@email.com" (plus anyone else who will authenticate with same client ID)
* Publishing status = "Testing"

Once the consent screen is configured, you should be able to create credentials at `APIs & Services > Credentials > Create Credentials > OAuth client ID` or `https://console.cloud.google.com/apis/credentials/oauthclient` and select "Desktop app" - an OAuth client should be created.  Download this client ID via the download button on the far right.

Once you have the client Id JSON file, you should be able to use the below to setup the rest of the configuration such as service accounts, scopes and roles.
