...
library(httr)
ga_id <- 1234557
res <- GET(paste0("https://slackbot-xxxx-ew.a.run.app/last-30-days?ga_id=",ga_id))
content(res)
```

Check the logs for Cloud Build and Cloud Run to see if its all working.

The first time I tried it the memory was too low for the app, which is why above it specifies "2G" for the Cloud Run size. 

But if all goes well then you should be able to see the CSV file come back.
...
Now we can schedule the URL via Cloud Scheduler.
...
You could try adding other endpoints for different GA functions, or making the API/webhook compatible with other services other than Slack.  You may also want to make the Cloud Run API private, and should if its controlling private of expensive operations, so then look at `cr_jwt_create()` or `cr_run_schedule_http()` as your private API options.
