...
If you want to run scripts that can be triggered one time, setup to trigger on GitHub events or pub/sub, or scheduled using Cloud Scheduler then [Cloud Build](https://code.markedmondson.me/googleCloudRunner/articles/cloudbuild.html) is suited to your use case. 
...
`cr_deploy_run()` wraps the above in functions to check and wait for status etc.

`cr_deploy_plumber()` adds checks for plumber API files.
