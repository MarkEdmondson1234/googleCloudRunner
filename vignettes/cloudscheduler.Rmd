...
Once the image is build successfully, you do not need to build it again for the scheduled calls - you could setup doing that only if the R code changes.  

The the scheduler, you will only need the image you build `gcr.io/your-project/your-name` and call it via the arguments set up in the Dockerfile i.e. `R -e my_r_script.R`

### 5. Make the build and optional source
...
schedule_me <- cr_build_yaml(
  steps = cr_build_step("your-name", 
                         "R -e my_r_script.R",
                         prefix="gcr.io/your-project")
                         )
                         
# maybe you want a source
...
### 6. Schedule calling the Docker image using Cloud Scheduler
